...
//Press 1 For Day Press 2 For Night
void display() {
...
void key_callback(unsigned char key, int x, int y) {
    if (key == 'D' || key == 'd') {
        currentScene = 1; // Set current scene to day mode
        glutPostRedisplay(); // Redraw the scene
    }
    else if (key == 'N' || key == 'n') {
        currentScene = 2; // Set current scene to night mode
        glutPostRedisplay(); // Redraw the scene
    }
}
...
int main(int argc, char** argv) {
    glutInit(&argc, argv);
    glutCreateWindow("Press d For Day Press n For Night");
